...
// ── Pull real journals from your DB later; hardcoded for now ──
const journalOptions = [
  { id: "j1", title: "African Journal of Social Sciences",      discipline: "Social Sciences",      scope: "Social dynamics, governance, and development" },
  { id: "j2", title: "Journal of African Health Research",      discipline: "Health Sciences",       scope: "Public health, epidemiology, clinical research" },
  { id: "j3", title: "African Economic Review",                  discipline: "Economics",             scope: "Economic policy, development, trade" },
  { id: "j4", title: "Journal of African Environmental Studies", discipline: "Environmental Science", scope: "Climate, biodiversity, sustainability" },
];
...
export default function SubmitForm() {
  ...
  // ── Step 1 — Journal
  const [selectedJournal, setSelectedJournal] = useState<{ id: string; title: string } | null>(null);
...
const [corrAuthor, setCorrAuthor] = useState({
  name: "", email: "", institution: "", country: "", orcid: "",
});
...
const [coAuthors, setCoAuthors] = useState<Array<{
  ...
  // ── Step 4 — Files (UI only for now — no S3 yet)
  ...
  const filteredJournals = journalOptions.filter(j =>
  ...
  const handleSubmit = async () => {
    ...
    if (!title.trim()) {
      toast({ title: "Missing title", description: "Please enter your manuscript title.", variant: "destructive" });
      return;
    }

    setIsSubmitting(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_NODE_API_URL}/api/manuscripts/submit`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          title, abstract, keywords, articleType, discipline,
        ...
        <div className="container max-w-4xl">
          ...
          {step === 1 && (
            <div className="space-y-6">
              ...
              <div className="relative">
                ...
                <Input placeholder="Search by name or discipline..." className="pl-10"
                  value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
              </div>
              <div className="grid gap-4">
                {filteredJournals.map(j => (
                  <Card key={j.id}
                    className={`cursor-pointer transition-all ${selectedJournal?.id === j.id ? "ring-2 ring-primary border-primary" : "hover:shadow-md"}`}
                    onClick={() => setSelectedJournal({ id: j.id, title: j.title })}
                  >
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start">
                        ...
                        {selectedJournal?.id === j.id && <CheckCircle className="h-5 w-5 text-primary" />}
                      </div>
                    </CardContent>
                  </Card>
                ))}
              ...
              <div className="flex justify-between">
                ...
                <Button onClick={next} disabled={!selectedJournal}>Continue <ArrowRight className="ml-2 h-4 w-4" /></Button>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Textarea value={abstract} onChange={e => setAbstract(e.target.value)}
                    placeholder="Enter your abstract..." className="min-h-[150px]" />
                </div>
                <div>
                  <Label>Keywords (comma-separated)</Label>
                  <Input value={keywords} onChange={e => setKeywords(e.target.value)}
                    placeholder="e.g., public health, governance, East Africa" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
              ...
              <div className="flex justify-between">
                <Button variant="outline" onClick={prev}><ArrowLeft className="mr-2 h-4 w-4" /> Back</Button>
                <Button onClick={next} disabled={!title.trim()}>Save & Continue <ArrowRight className="ml-2 h-4 w-4" /></Button>
              </div>
            </div>
          )}
              ...
              <Card><CardContent className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div><Label>Full Name</Label>
                    <Input value={corrAuthor.name} onChange={e => setCorrAuthor({ ...corrAuthor, name: e.target.value })} placeholder="Dr. Jane Doe" /></div>
                  <div><Label>Email</Label>
                    <Input type="email" value={corrAuthor.email} onChange={e => setCorrAuthor({ ...corrAuthor, email: e.target.value })} placeholder="jane@university.edu" /></div>
                  <div><Label>Institution</Label>
                    <Input value={corrAuthor.institution} onChange={e => setCorrAuthor({ ...corrAuthor, institution: e.target.value })} placeholder="University of Nairobi" /></div>
                  <div><Label>Country</Label>
                    <Input value={corrAuthor.country} onChange={e => setCorrAuthor({ ...corrAuthor, country: e.target.value })} placeholder="Kenya" /></div>
                  <div className="md:col-span-2"><Label>ORCID ID (optional)</Label>
                    <Input value={corrAuthor.orcid} onChange={e => setCorrAuthor({ ...corrAuthor, orcid: e.target.value })} placeholder="0000-0000-0000-0000" /></div>
                </div>
              </CardContent></Card>
              ...
              <Card><CardContent className="p-6 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <Button size="sm" variant="outline"
                    onClick={() => setCoAuthors([...coAuthors, { name: "", email: "", institution: "", role: "" }])}>
                    <Plus className="h-4 w-4 mr-1" /> Add Co-Author
                  </Button>
                </div>
                {coAuthors.map((ca, i) => (
                  <div key={i} className="border rounded-lg p-4 space-y-3 relative">
                    <button onClick={() => setCoAuthors(coAuthors.filter((_, j) => j !== i))}
                      className="absolute top-2 right-2 text-muted-foreground hover:text-destructive">
                      <X className="h-4 w-4" />
                    </button>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div><Label>Name</Label>
                        <Input value={ca.name} onChange={e => setCoAuthors(coAuthors.map((c, j) => j === i ? { ...c, name: e.target.value } : c))} placeholder="Co-author name" /></div>
                      <div><Label>Email</Label>
                        <Input value={ca.email} onChange={e => setCoAuthors(coAuthors.map((c, j) => j === i ? { ...c, email: e.target.value } : c))} placeholder="Email" /></div>
                      <div><Label>Institution</Label>
                        <Input value={ca.institution} onChange={e => setCoAuthors(coAuthors.map((c, j) => j === i ? { ...c, institution: e.target.value } : c))} placeholder="Institution" /></div>
                      <div><Label>Contribution Role</Label>
                        <Select value={ca.role} onValueChange={v => setCoAuthors(coAuthors.map((c, j) => j === i ? { ...c, role: v } : c))}>
                ...
                {["Main Manuscript", "Cover Letter", "Declaration of Originality"].map(f => (
                  <Card key={f} className={uploadedFiles[f] ? "border-primary/50 bg-primary/5" : ""}>
                    <CardContent className="p-4 flex items-center justify-between">
                      ...
                      <Button size="sm" variant={uploadedFiles[f] ? "outline" : "default"}
                        onClick={() => setUploadedFiles({ ...uploadedFiles, [f]: !uploadedFiles[f] })}>
                ...
                {["Figures/Tables", "Supplementary Data", "Ethics Approval"].map(f => (
                  <Card key={f} className={uploadedFiles[f] ? "border-primary/50 bg-primary/5" : ""}>
                    <CardContent className="p-4 flex items-center justify-between">
                      ...
                      <Button size="sm" variant={uploadedFiles[f] ? "outline" : "secondary"}
                        onClick={() => setUploadedFiles({ ...uploadedFiles, [f]: !uploadedFiles[f] })}>
                        {uploadedFiles[f] ? "✔ Uploaded" : "Upload"}
                      </Button>
                    </CardContent>
          ...
          {step === 5 && (
            <div className="space-y-6">
              ...
              <Card><CardContent className="p-6 space-y-4">
                {[
                  ...
                  <div key={d.id} className="flex items-start gap-3">
                    <Checkbox id={d.id} checked={declarations[d.id] || false}
                      onCheckedChange={v => setDeclarations({ ...declarations, [d.id]: !!v })} />
              ...
              <div className="flex justify-between">
                <Button variant="outline" onClick={prev}><ArrowLeft className="mr-2 h-4 w-4" /> Back</Button>
                <Button onClick={handleSubmit} disabled={isSubmitting} className="bg-primary font-semibold">
          ...
          {/* ── Step 6 — Confirmation (real submissionId) ── */}
          {step === 6 && (
            <div className="text-center py-12 space-y-6">
              ...
              {submissionId && (
                <p className="text-sm font-mono bg-muted inline-block px-4 py-2 rounded-lg">
              ...
              <div className="flex justify-center gap-4 pt-4">
                ...
                <Button variant="outline" onClick={() => { setStep(0); setTitle(""); setAbstract(""); setKeywords(""); setSelectedJournal(null); }}>
