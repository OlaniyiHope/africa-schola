              ...
              {allProfiles.length > 0 ? (
                    ...
                    <div key={p.user_id} className="bg-card rounded-xl border border-border p-4 flex items-center gap-3">
                      ...
                      <div className="flex flex-col gap-1 shrink-0">
                        ...
                        {!connectedUserIds.has(p.user_id) && !pendingUserIds.has(p.user_id) ? (
                          <Button  size="sm" className="text-xs gap-1" onClick={() => sendConnectionRequest(p.user_id)}>
                ...
                <div className="bg-card rounded-xl border border-border p-12 text-center">
                  ...
                  <Link to="/dashboard/profile">
                    <Button  size="sm" className="gap-1.5"><User className="h-3.5 w-3.5" /> Complete Your Profile</Button>
                  </Link>
                </div>
              )}
            </div>
          ...
          <div>
            {applications.length > 0 ? (
              ...
              <div className="bg-card rounded-xl border border-border p-12 text-center">
                ...
                <Button  size="sm" className="mt-3 gap-1.5" onClick={() => setActiveTab("jobs")}>
      ...
      <Dialog open={advisoryDialog} onOpenChange={setAdvisoryDialog}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button  size="sm" disabled={saving || !advisoryForm.topic.trim()} onClick={async () => {
              setSaving(true);
      ...
      <Dialog open={applyDialog} onOpenChange={setApplyDialog}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button  size="sm" disabled={saving || !applyForm.research_statement.trim()} onClick={async () => {
              setSaving(true);
