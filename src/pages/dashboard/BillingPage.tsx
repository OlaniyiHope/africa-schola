...
import { CreditsHowItWorksModal } from "./CreditsModal";
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <Dialog>
              <DialogTrigger asChild>
                <Button  size="sm" className="gap-1"><ArrowRight className="h-3 w-3" /> Upgrade Plan</Button>
              </DialogTrigger>
          ...
          {billingAlerts.map((alert, i) => (
            <div key={i} className="bg-card rounded-xl border border-border p-4 flex items-center justify-between gap-3">
              ...
              <Link to={alert.ctaUrl}>
                <Button size="sm" className="shrink-0 text-xs">{alert.cta}</Button>
              </Link>
            </div>
          ))}
        </div>
          ...
          {purchaseState === "error" && (
            <div className="bg-destructive/10 border border-destructive/30 rounded-xl p-4 flex items-center justify-between gap-3">
              ...
              <div className="flex gap-2 shrink-0">
                <Button  size="sm" onClick={() => setPurchaseState("idle")}>Retry</Button>
                <Link to="/dashboard/billing/payment-methods">
                  <Button  size="sm">Change Method</Button>
                </Link>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {creditPacks.map((p) => (
              <div key={p.label} className="bg-card rounded-xl border border-border p-5 text-center space-y-3">
                ...
                <Button  size="sm" className="w-full" onClick={() => handlePurchase(p.label)}>Purchase</Button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-card rounded-xl border border-border p-6 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Button  size="sm" className="gap-1" onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(`Join Afrika Scholar using my referral link: ${referralLink}`)}`, "_blank")}>
                <ExternalLink className="h-3 w-3" /> WhatsApp
              </Button>
              <Button  size="sm" className="gap-1" onClick={() => window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(referralLink)}`, "_blank")}>
                <ExternalLink className="h-3 w-3" /> LinkedIn
              </Button>
              <Button  size="sm" className="gap-1" onClick={() => window.open(`mailto:?subject=${encodeURIComponent("Join Afrika Scholar")}&body=${encodeURIComponent(`Check out Afrika Scholar: ${referralLink}`)}`)}>
          ...
          <div className="bg-card rounded-xl border border-border p-6 space-y-4">
            ...
            {spendingLimitEnabled && (
              <div className="space-y-2">
                ...
                <Button  size="sm" onClick={() => toast({ title: "Spending limit saved" })}>
                  Save Limit
                </Button>
              </div>
            )}
