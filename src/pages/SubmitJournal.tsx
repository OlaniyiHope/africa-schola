...
//   try {
//     const res = await fetch("/api/journals/submit", {
//       method: "POST",
//       headers: {
...
//     setState(prev => ({ ...prev, submitted: true, submissionId: data.submissionId }));
//     setIsSubmitted(true);
...
import {
  ArrowLeft, CheckCircle, BookOpen, Users, ShieldCheck,
  Scale, FileText, Send, Mail, AlertCircle,
} from "lucide-react";
import { useJournal } from "@/context/JournalContext";
...
function ReviewRow({ label, value }: { label: string; value?: string }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "1rem", padding: "0.6rem 0", borderBottom: "1px solid #f3f4f6" }}>
      <span style={{ fontSize: "0.8rem", color: "var(--muted-foreground)", fontWeight: 600, flexShrink: 0 }}>{label}</span>
      <span style={{ fontSize: "0.83rem", color: "var(--foreground)", textAlign: "right", fontWeight: 500 }}>
        {value || <span style={{ color: "#d1d5db" }}>Not set</span>}
      </span>
    </div>
  );
}
...
export default function SubmitJournal() {
  ...
  // ✅ Correctly typed — these now exist on JournalState
...
const handleSubmit = async () => {
  ...
  if (!token) {
    toast({ 
      title: "Not logged in", 
      description: "Please log in to submit a journal.", 
      variant: "destructive" 
    });
    navigate("/login");
    return;
  ...
  try {
    const res = await fetch(`${import.meta.env.VITE_NODE_API_URL}/api/journals/submit`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`, // ✅ use variable not inline localStorage
      },
      body: JSON.stringify({
        ...p,
        governanceComplete: state.governanceComplete,
      ...
      <Layout>
        <div style={{ minHeight: "70vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "4rem 1rem" }}>
          <div style={{ maxWidth: 540, width: "100%", background: "#fff", border: "1px solid #e5e7eb", borderRadius: 20, padding: "3rem 2.5rem", textAlign: "center", boxShadow: "0 8px 40px rgba(0,0,0,0.07)" }}>
            ...
            <p style={{ fontSize: "0.95rem", color: "var(--muted-foreground)", lineHeight: 1.7, margin: "0 0 0.75rem" }}>
              Your journal proposal has been received. We'll review it and respond within{" "}
              <strong style={{ color: "var(--foreground)" }}>5–7 working days</strong>.
            </p>
            <div style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 8, padding: "0.5rem 1rem", fontSize: "0.8rem", color: "var(--muted-foreground)", marginBottom: "2rem" }}>
              <FileText size={13} />
              Submission ID:{" "}
              <strong style={{ color: "var(--foreground)", fontFamily: "monospace" }}>{submissionId}</strong>
            </div>
    ...
    <Layout>
      <div className="py-8">
        ...
        <div className="mx-auto max-w-3xl space-y-6 px-4 mt-10">
          ...
          <div className="text-center">
            <h1 className="font-serif text-3xl font-bold text-primary">Review & Submit</h1>
            <p className="mt-2 text-muted-foreground">
              Review your journal setup before submitting to the Afrika Scholar editorial team.
            </p>
          </div>
          ...
          <Card>
            ...
            <CardContent>
              <SectionReview icon={BookOpen} title="Journal Details">
                <ReviewRow label="Journal Title"   value={p.title} />
                <ReviewRow label="Discipline"      value={p.discipline} />
                <ReviewRow label="Sub-discipline"  value={p.subDiscipline} />
                <ReviewRow label="Institution"     value={p.institution} />
                <ReviewRow label="Contact Email"   value={p.contactEmail} />
                <ReviewRow label="Country"         value={p.country} />
                <ReviewRow label="Website"         value={p.website} />
              </SectionReview>

              <SectionReview icon={Users} title="Editorial Leadership">
                <ReviewRow label="Editor-in-Chief"    value={p.editorName} />
                <ReviewRow label="Editor Title"       value={p.editorTitle} />
                <ReviewRow label="Editor Affiliation" value={p.editorAffiliation} />
                <ReviewRow
                  label="Board Members"
                  value={
                    p.boardMembers.filter(m => m.name.trim()).length > 0
                      ? `${p.boardMembers.filter(m => m.name.trim()).length} members added`
                      : undefined
                  }
                />
              </SectionReview>

              <SectionReview icon={BookOpen} title="Publishing Model">
                <ReviewRow label="Frequency"   value={p.frequency} />
                <ReviewRow label="Language(s)" value={p.languages} />
              </SectionReview>

              <SectionReview icon={Users} title="Editorial Governance">
                <ReviewRow
                  label="Governance Confirmed"
                  value={state.governanceComplete ? "✓ Complete" : "Incomplete"}
                />
              </SectionReview>
              ...
              <SectionReview icon={ShieldCheck} title="Policies">
                <ReviewRow
                  ...
                  value={pol.reviewType.replace(/-/g, " ").replace(/\b\w/g, c => c.toUpperCase())}
                />
                <ReviewRow label="Open Access License"  value={pol.license.toUpperCase()} />
                <ReviewRow label="Embargo Period"        value={pol.embargo} />
                <ReviewRow label="Referencing Style"     value={pol.refStyle} />
                <ReviewRow label="Plagiarism Screening"  value={pol.plagiarismTool ? "Required" : "Optional"} />
                <ReviewRow label="AI Disclosure"         value={pol.aiDisclosure ? "Required" : "Optional"} />
                <ReviewRow label="Data Sharing"          value={pol.dataSharing ? "Required" : "Optional"} />
              </SectionReview>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.625rem" }}>
                {[
                  { label: "Journal title provided",         done: !!p.title },
                  { label: "Discipline selected",            done: !!p.discipline },
                  { label: "Editor-in-Chief named",          done: !!p.editorName },
                  { label: "Contact email provided",         done: !!p.contactEmail },
                  { label: "3+ board members added",         done: p.boardMembers.filter(m => m.name.trim()).length >= 3 },
                  { label: "Ethical commitments accepted",   done: p.commitPeerReview && p.commitEthics && p.commitNoPredatory },
                  { label: "Editorial governance confirmed", done: state.governanceComplete },
                  { label: "Journal policies defined",       done: state.policiesComplete },
                ].map(({ label, done }) => (
                  <div
                    ...
                    style={{
                      ...
                      background: done ? "rgba(34,197,94,0.05)" : "rgba(239,68,68,0.04)",
                  ...
                  >
                    ...
                    <span style={{ fontSize: "0.85rem", fontWeight: 600, color: done ? "var(--foreground)" : "#ef4444" }}>
                      {label}
                    </span>
                  </div>
                ))}
